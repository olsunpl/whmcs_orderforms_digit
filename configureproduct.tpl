        ...
        <div class="row">
          <div class="secondary-cart-body">
            ...
            {if $pricing.type eq "recurring"}
              <div class="field-container">
                <div class="form-group">
                  <label class="wdes-product-heading-options" for="inputBillingcycle">{$LANG.cartchoosecycle}</label>
                  <select name="billingcycle" id="inputBillingcycle" class="form-control select-inline custom-select"
                    onchange="{if $configurableoptions}updateConfigurableOptions({$i}, this.value);{else}recalctotals();{/if}">
                    {if $pricing.monthly}
            ...
            {if $productinfo.type eq "server"}
              ...
              <div class="field-container wdes-server-field-container">
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <input type="text" name="hostname" class="form-control" id="inputHostname"
                        value="{$server.hostname}" placeholder="servername.yourdomain.com">
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="form-group">
                ...
                <div class="row">
                  {foreach $configurableoptions as $num => $configoption}
                    {if $configoption.optiontype eq 1}
                      <div class="col-sm-6">
                        <div class="form-group">
                          <label class="wdes-main-label"
                            for="inputConfigOption{$configoption.id}">{$configoption.optionname}</label>
                          <select name="configoption[{$configoption.id}]" id="inputConfigOption{$configoption.id}"
                            class="form-control">
                            {foreach key=num2 item=options from=$configoption.options}
                              <option value="{$options.id}" {if $configoption.selectedvalue eq $options.id} selected="selected"
                                {/if}>
                                {$options.name}
                              </option>
                            {/foreach}
                    ...
                    {elseif $configoption.optiontype eq 2}
                      <div class="col-sm-6">
                        <div class="form-group">
                          ...
                          {foreach key=num2 item=options from=$configoption.options}
                            <br />
                            <label class="wdes-item-option">
                              <input type="radio" name="configoption[{$configoption.id}]" value="{$options.id}"
                                {if $configoption.selectedvalue eq $options.id} checked="checked" {/if} />
                              {if $options.name}
                                {$options.name}
                              {else}
                                {$LANG.enable}
                        ...
                        <div class="form-group">
                          ...
                          <label>
                            <input type="checkbox" name="configoption[{$configoption.id}]"
                              id="inputConfigOption{$configoption.id}" value="1" {if $configoption.selectedqty}
                              checked{/if} />
            ...
            <div class="alert alert-warning info-text-sm">
              ...
              {$LANG.orderForm.haveQuestionsContact} <a href="contact.php" target="_blank"
